...
class TheoryElement(metaclass=ABCMeta):
    ...
    def __init__(self,DT,CDT,Hangars,BR,Movement,Tag,Capital,Size,BC,faction,ld,SizeFactor):
        ...
        self.aim = vct.vector_from_dots((0,0),(1,1))               #Direction pointée par l'unité
    ...
    @property
    def CDamageTrack(self,n):        #Récupère la DT pour calculs internes
    ...
    @CDamageTrack.setter
    def set_CDamageTrack(self,L):            #Avec L la liste de 3 entiers, nouvelle DT
        self._DamageTrack=L
    ...
    @aim.setter
    def set_aim(self,theta):
        if theta>self.aim+45:
            raise ValueError("Angle non valide")
        else:
            self.aim=theta
